/**
 * Created by sbt-khruzin-mm on 28.04.2017.
 */
...
public class IgniteClient {
    ...
    public int AcctTransfer(AcctTransfer transfer){
        //System.out.format("TXStart.Client1 name (from): %s, client2 name (to): %s sum: %d %n", transfer.AccountFrom, transfer.AccountTo, transfer.TransferAmount);
        try (Transaction tx = ignite.transactions().txStart(PESSIMISTIC, SERIALIZABLE, 10000, 0)) {
            ...
            tx.commit();
            //System.out.format("TXEnd.Client1 name (from): %s, client2 name (to): %s sum: %d %n", transfer.AccountFrom, transfer.AccountTo, transfer.TransferAmount);
        }
        catch (Throwable e){
